(function() {
    ...
    if (!document.getElementById('chatbot-widget-container')) {
        ...
        // 檢查頁面有沒有 Tailwind，如果沒有，這裡可以動態插入 Tailwind CDN (選用)
        // 但因為您的專案已經都有 Tailwind，這裡就省略
    }
    ...
    function appendMessage(text, sender, needSave = true) {
        ...
        div.className = sender === 'user' ? 'flex justify-end animate-fade-in' : 'flex justify-start animate-fade-in';
        
        const contentHTML = sender === 'user' 
            ? `<div class="flex items-end gap-2 max-w-[85%] flex-row-reverse">
                 <div class="bg-[#e50914] text-white px-4 py-2.5 rounded-2xl rounded-br-none shadow-md text-sm leading-relaxed text-left">${text.replace(/\n/g, '<br>')}</div>
               </div>`
            : `<div class="flex items-end gap-2 max-w-[85%]">
                 <div class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center shrink-0 overflow-hidden"><span class="text-xs">🤖</span></div>
                 <div class="bg-white border border-gray-200 text-gray-700 px-4 py-2.5 rounded-2xl rounded-bl-none shadow-sm text-sm leading-relaxed">${text.replace(/\n/g, '<br>')}</div>
               </div>`;
    ...
    function showLoading() {
        ...
        div.innerHTML = `
            <div class="bg-gray-100 px-4 py-3 rounded-2xl rounded-bl-none flex space-x-1 items-center ml-10">
